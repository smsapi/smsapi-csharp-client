...
{
    public enum SmsFallbacks
    {
        [EnumMember(Value = "vms")] Vms
    }

    private const string Encoding = "UTF-8";
    private SmsFallbacks? _fallback;
    ...
    */
    ...
    public SMSSend SetParam(int i, string text)
    {
        if (i > 3 || i < 0) throw new IndexOutOfRangeException();

        if (@params == null) @params = new string[4];
    ...
    public SMSSend WithFallback(SmsFallbacks fallback)
    {
        _fallback = fallback;
    ...
    {
        if (text == null && template == null) throw new ArgumentException("Cannot send message without text!");
    }
    ...
    protected override (NameValueCollection, ISet<KeyValuePair<string, dynamic?>>?) Values()
    {
        ...
        if (sender != null) collection.Add("from", sender);

        if (To != null) collection.Add("to", string.Join(",", To));

        if (Group != null) collection.Add("group", Group);
        ...
        if (dataCoding != null) collection.Add("datacoding", dataCoding);

        if (maxParts > 0) collection.Add("max_parts", maxParts.ToString());

        if (DateSent != null) collection.Add("date", DateSent);

        if (dateExpire != null) collection.Add("expiration_date", dateExpire);

        if (Partner != null) collection.Add("partner_id", Partner);

        collection.Add("encoding", Encoding);

        if (normalize) collection.Add("normalize", "1");

        if (Test) collection.Add("test", "1");
        ...
        if (template != null) collection.Add("template", template);

        return (collection, PrepareRequestBody());
    }

    private ISet<KeyValuePair<string, dynamic?>>? PrepareRequestBody()
    {
        ISet<KeyValuePair<string, dynamic?>>? values = null;

        _fallback?.Let(fallback =>
        {
            var fallbacks = new HashSet<Dictionary<string, string>>
                { new() { { "type", fallback.GetEnumValue() } } };

            values = new HashSet<KeyValuePair<string, dynamic>>
            {
                new("fallback", fallbacks)
            };
        });

...
[TestClass]
public class UpdateShortUrlResponseTest
{
    private readonly ProxyStub _proxyStub = new();

    [TestMethod]
    public void update_short_url()
    {
        ...
        var result = UpdateShortUrl().Execute();
    ...
    [TestMethod]
    public void map_not_found_when_updating()
    {
        _proxyStub.SyncExecutionResponse = new HttpResponseEntity(
        ...
        var action = () => { _ = UpdateShortUrl().Execute(); };

        Assert.ThrowsException<NotFoundException>(action);
    }

    private UpdateShortUrl UpdateShortUrl()
    {
        var action = new UpdateShortUrl("any id");
